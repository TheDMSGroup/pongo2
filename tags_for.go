...
func (node *tagForNode) Execute(ctx *ExecutionContext, writer TemplateWriter) (forError *Error) {
	...
	parentloop, _ := forCtx.Private.GetValue("forloop")
	...
	forCtx.Private.SetValue("forloop", loopInfo)
	...
	obj.IterateOrder(func(idx, count int, key, value *Value) bool {
		...
		forCtx.Private.SetValue(node.key, key)
		if value != nil {
			forCtx.Private.SetValue(node.value, value)
		}
